...
Polyarb is a research-only Polymarket scanner for structural anomalies:
...
For each candidate, human output includes:
...
JSON output includes the same information in a structured format:

```bash
polyarb scan --json --limit-events 50
```

See `examples/sample_output.json` for the shape.

## Data Sources

Polyarb uses only public read-only endpoints:
...
- real-like Gamma payload parsing
- neg-risk basket math
- underround and overround detection
- executable basket-cost estimation from mocked books
...
- This is a research scanner, not proof of a risk-free trade.
- Polymarket rules, market wording, fees, and resolution edge cases can dominate apparent arithmetic.
